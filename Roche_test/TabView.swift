...
internal class TabView: UIView {
    ...
    var pageTabItems: [NSMutableAttributedString] = [] {
        didSet {
    ...
    fileprivate var InteritemSpacing: CGFloat = 20
    ...
    fileprivate var lastSize: CGFloat = 0
    ...
    func scrollCurrentBarView(_ index: Int, contentOffsetX: CGFloat) {
        ...
        if let currentCell = collectionView.cellForItem(at: currentIndexPath) as? TabCollectionCell, let nextCell = collectionView.cellForItem(at: nextIndexPath) as? TabCollectionCell {
            ...
            if isInfinity {
                ...
                if scrollRate > 0 {
                currentBarViewLeftConstraint?.constant = currentCell.frame.minX + scrollRate * currentCell.frame.width + InteritemSpacing
                } else {
                    currentBarViewLeftConstraint?.constant = currentCell.frame.minX + nextCell.frame.width * scrollRate - InteritemSpacing
                }
            }
...
//        collectionView.contentInset = UIEdgeInsets(top: 0, left: 20, bottom: 0, right: 0)
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAt section: Int) -> UIEdgeInsets {
        return UIEdgeInsets(top: 0, left: 20, bottom: 0, right: UIScreen.main.bounds.width/2 - lastSize/2)
    }

    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, minimumInteritemSpacingForSectionAt section: Int) -> CGFloat {
//        collectionView.setContentOffset(CGPoint(x: 50, y: 0) , animated: false)
